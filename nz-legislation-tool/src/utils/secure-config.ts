...
function ensureSecureConfigDir() {
  if (!existsSync(CONFIG_DIR)) {
    ...
    try {
      ...
      logger.warn('Failed to set secure permissions on config directory', error instanceof Error ? { error: error.message } : undefined);
    }
  } else {
    ...
    try {
      ...
      logger.warn('Failed to verify config directory permissions', error instanceof Error ? { error: error.message } : undefined);
    }
  }
}
