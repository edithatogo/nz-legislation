...
export class StreamExporter {
  ...
  async exportWorkDetails(
    ...
    onProgress?: (progress: StreamProgress) => void
  ): Promise<{ processed: number; bytesWritten: number; failed: Array<{ workId: string; error: string }> }> {
    ...
    let processed = 0;
    let bytesWritten = 0;
    const failedItems: Array<{ workId: string; error: string }> = [];
    ...
    try {
      ...
      for (let i = 0; i < workIds.length; i++) {
        ...
        try {
          ...
          failedItems.push({ workId, error: (error as Error).message });
          // Emit error event if listener is provided
          if (onProgress) {
            // Note: In a full implementation, we would use EventEmitter here
            // For now, just log the error with context
            console.warn(`⚠️ Failed to fetch work ${workId}: ${(error as Error).message}`);
          }
        }
      ...
      if (failedItems.length > 0) {
        ...
        writeFileSync(errorPath, JSON.stringify(failedItems, null, 2));
        console.warn(`⚠️ ${failedItems.length} items failed to export. See ${errorPath} for details`);
      }

      return { processed, bytesWritten, failed: failedItems };
    } catch (error) {
      writeStream.destroy();
      throw error;
    }
  ...
  private async writeMetadata(params: SearchParams, totalResults: number): Promise<void> {
    ...
    if (this.format === 'csv') {
      // Write metadata to separate file for CSV to avoid breaking CSV parsing
      const { writeFileSync } = await import('fs');
      const metaPath = this.outputPath.replace('.csv', '.meta.json');
