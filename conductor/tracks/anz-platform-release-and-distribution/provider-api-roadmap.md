...
| Queensland                           | Queensland Legislation API service                                        | P0       | Official API located; account registration, terms, Swagger shape, and test fixtures still required | Replace empty/stub Queensland provider behavior with source-backed support or explicit unsupported errors.      |
| New South Wales                      | NSW legislation XML export and bulk download surface                      | P1       | Official XML/export surface located; API-equivalent adapter shape still required                   | Add discovery spike, access review, capability manifest entry, and provider tests before runtime enablement.    |
...
record confirms the official API surface, and the Australian source inventory
records the other official source surfaces located during follow-up discovery.
Runtime support remains blocked until provider adapter mapping and tests land
